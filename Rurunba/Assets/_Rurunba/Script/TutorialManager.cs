...
public class TutorialManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 初期状態ではステージ全体の吸い込み口を強制的に無効化（誤作動・置き去りバグ防止）
        SetSuctionZonesActive(false);

        ShowOpeningLine();
        SetPlayerScriptEnabled(false);
        SetPlayerPhysicsLocked(true); // 最初は物理機能をロックして固定
    }
    ...
    // すべての吸い込み口（コライダー）の有効・無効を確実に切り替えるメソッド
    ...
    // 落下中のスリープ対策（着地するまでの間、定期的にゴミを起こし続ける）
    ...
    void Update()
    {
        ...
        {
            SetPlayerScriptEnabled(false);
            SetPlayerPhysicsLocked(true); // transform書き換えを廃止し、物理機能で固定（子コライダーが置き去りにならない）
            HandleNovelInput();
            return;
        }

        // プレイヤーが動くフェーズに入ったら物理固定を解除する
    ...
    // 物理エンジンと完全に同期したまま安全に位置を固定・解除するメソッド
    void SetPlayerPhysicsLocked(bool isLocked)
    {
        ...
        if (playerObject != null)
        {
            ...
            if (rb != null)
            {
                rb.isKinematic = isLocked; // trueにすると物理演算が止まり、その場に完全固定される（判定バグが起きない）
                if (isLocked)
                {
    ...
    IEnumerator CountdownRoutine()
    {
        ...
        SetPlayerPhysicsLocked(false); // 物理ロック解除
    ...
    void HandleGyroPhase()
    {
        ...
        if (gyroTimer >= 5f)
        {
            ...
            // ここではまだ吸い込みはONにしない（フライングを完全に防ぐため）
    ...
    IEnumerator ClearPhase2Routine()
    {
        ...
        // ゴミ集めフェーズ（Phase3）が始まるまさにこの瞬間に、初めて吸い込み判定をONにする
    ...
    IEnumerator TutorialGoalSequence()
    {
        ...
        SetPlayerScriptEnabled(false);
        SetPlayerPhysicsLocked(true); // ゴール演出中も物理を安全にロック
